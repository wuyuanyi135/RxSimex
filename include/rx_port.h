...
class port_base {
 ...
 public:
  /// copy port data to destination buffer
  /// \param dest
  /// \param size
  virtual void to(void *dest, size_t size = -1) = 0;

  ///
  /// \param src input signal ptr. Note that the pointer is doubled for
  /// non-contiguous data.
  /// \param size
  ...
  /// \param src
  /// \param size
  virtual void from(const void *src, size_t size = -1) = 0;
...
class port : public port_base {
 public:
  ...
  explicit port() {
    auto subscriber =
        rxcpp::make_subscriber<ArrayT>(_subscription, [this](ArrayT arr) {
  ...
  virtual ~port() { _subscription.unsubscribe(); }
 ...
 public:
  ...
  void from(const void *src, size_t size = -1) override {
    if (size == -1) {
      size = get_width();
    }
    std::lock_guard lock(mutex);
  ...
  size_t get_width() override { return sizeof(T) * get_num_elements(); }
  ...
  size_t get_num_elements() override { return xt::prod(dims)(0); }
