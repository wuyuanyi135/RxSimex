...
void mdlCheckParameters(SimStruct *S) {
  ...
  for (int i = 0; i < block->dialog_params.size(); i++) {
    if (!block->dialog_params[i]->update_parameter(ssGetSFcnParam(S, i))) {
      throw std::invalid_argument(std::string("failed to update param #") +
                                  std::to_string(i) +
                                  " due to type assertion failure");
    }
  }

  block->on_parameter_updated();
}
...
extern "C" void mdlInitializeSizes(SimStruct *S) {
  ...
  if (ssGetNumSFcnParams(S) == ssGetSFcnParamsCount(S)) {
    ...
    throw std::invalid_argument(
        "Number of parameter mismatch"); /* Parameter mismatch reported by the
                                            Simulink engine*/
  }
  ...
  for (int i = 0; i < block->dialog_params.size(); i++) {
    ssSetSFcnParamTunable(
        S, i,
        block->dialog_params[i]->tunable ? SS_PRM_TUNABLE : SS_PRM_NOT_TUNABLE);
  }
...
extern "C" void mdlSetInputPortDimensionInfo(SimStruct *S, int_T port,
                                             const DimsInfo_T *dimsInfo) {
  auto block = simex::rx_block::instance;
  assert(block);
  auto &p = block->input_ports[port];
  std::vector<std::size_t> shape = {
      static_cast<unsigned long>(dimsInfo->numDims)};
  auto dims =
      xt::adapt(dimsInfo->dims, dimsInfo->numDims, xt::no_ownership(), shape);
  p->dims = dims;
  p->on_dimension_update();
  ssSetInputPortDimensionInfo(S, port, dimsInfo);
}
#define MDL_SET_OUTPUT_PORT_DIMENSION_INFO
extern "C" void mdlSetOutputPortDimensionInfo(SimStruct *S, int_T port,
                                              const DimsInfo_T *dimsInfo) {
  auto block = simex::rx_block::instance;
  assert(block);
...
void mdlInitializeConditions(SimStruct *S) {
  ...
  block->on_initial_condition(ssIsFirstInitCond(S));
}
